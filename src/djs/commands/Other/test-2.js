...
module.exports = {
  ...
  async execute(interaction) {
     ...
     if (duration !=== null) {
     // call a start function and pass the master value of true
     // in the other function if master is true, continue eith the duration, if false stop the messaging
         generate(interaction, duration, "start")
     } else {
         generateLinks()
     }
    ...
    function generate( i, time, toggle ) {
      const channel = i.channel;
      const userId = i.user.id;
      
      // calculate how many messages I can send within the duration given a 1 minute time buffer
