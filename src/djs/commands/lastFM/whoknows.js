    ...
    async execute(message, args) {
        ...
        for (const member of members) {
            try{
                ...
                if (LFuser) {
                    const uri = `https://ws.audioscrobbler.com/2.0/?method=artist.getInfo&user=${LFuser.lastfmID}&api_key=2fdf8c5b06054003142716d7a970cada&artist=${artist}`
                    ...
                    userplays = dom1.window.document.querySelector('stats userplaycount').textContent;
                    artistname = dom1.window.document.querySelector('name').textContent;

                    topCollect.push({ username: message.guild.members.cache.get(member).user.tag, playcount: userplays, LFname: LFuser.lastfmID })
                    if (topCollect.length > 99) break;
                }
            } catch(err) {
                console.log(err);
            }
        ...
        const pageSelect = async (page) => {
            ...
            const embed1 = new EmbedBuilder()
                ...
                .setTitle(`Top Listeners | ${artistname}`)
