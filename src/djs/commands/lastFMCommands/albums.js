...
module.exports = {
    ...
    .setName("talbum-lastfm")
    .setDescription("Top Albums - LastFM")
        ...
        try {
          ...
          try{
            ...
            if(err.message.includes(`buffering timed out`)){
              ...
              let uri = `https://ws.audioscrobbler.com/2.0/?method=user.gettopalbums&user=${LFuser?.lastfmID}&api_key=${lastFM_API_ID}&limit=10`
            ...
            const topalbums = await axios.get(uri);

            const dom1 = new jsdom.JSDOM(topalbums.data, {
              contentType: "text/xml",
            });
            
            const embed = {
              color: userinfoget.hexAccentColor,
              title: `${LFuser?.lastfmID} | Top Albums (${timelength})`,
              url: `https://www.last.fm/user/${LFuser?.lastfmID}`,
              footer: {
                text: `Requested by: ${interaction.user.username} | Total Plays: ${calculateTotalPlays(dom1)}`,
            ...
            let albumNodes = dom1.window.document.querySelectorAll('album');
            let description = '';
            if (albumNodes.length > 0) {
              for (let i = 0; i < albumNodes.length; i++) {
                let album = albumNodes[i].querySelector('name').textContent;
                let artist = albumNodes[i].querySelector('artist name').textContent;
                let url = albumNodes[i].querySelector('url').textContent;
                let play = albumNodes[i].querySelector('playcount').textContent;
                let rank = i + 1;
            
                if (description.length + `\`${rank}.\` **[${album}](${url})** by **[ *${artist}* ]** (\`${play <= 1 ? `${play} Play` : `${play} Plays`}\`)\n`.length > 4000) {
                  let remainingAlbums = albumNodes.length - i;
                  description += `...and ${remainingAlbums} more`;
                  break;
                }
            
                description += `\`${rank}.\` **[${album}](${url})** by **[ *${artist}* ]** (\`${play <= 1 ? `${play} Play` : `${play} Plays`}\`)\n`;
              }
            
              embed.description = description;
              if (description.includes('more')) {
                embed.description += `\n\nNote: Some albums were not displayed due to the 4096 character limit.`;
              }
            } else {
              embed.description = 'No albums were found';
            }
